...
function initMenu(indexId){
//	alert("index="+indexId);
	var root = $(window.frames[0].document).find("#menu").children("ul").children("li");
	root.children("ul").slideUp();
	root.children("a").children("span").attr("class", "shousuo");
	
	var selectmenu = $(window.frames[0].document).find("#"+indexId+"").children("a");
	var children = selectmenu.next("ul");
	var span = selectmenu.children("span");
...
//	$(window.frames[0].document).find("#dataul").show();
}
